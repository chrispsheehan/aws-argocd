...
sudo su -s /bin/bash -c "argocd app create test --server localhost:8999 --dest-namespace argocd --dest-server https://kubernetes.default.svc --repo https://github.com/chrispsheehan/aws-argocd --path k8s --revision main --sync-policy automated" ec2-user
